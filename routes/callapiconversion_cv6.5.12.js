...
var express = require('express'),
    ...
    function AX2020_callback(res, outputFormat, contentLocation, callbackMethod, tempFileName){
        ...
        try{	
            ...
            if (config.cvjs_debug) console.log("AX2020_callback  - global counter: "+globalCounter+" outputFormat:"+outputFormat);
            
            if (outputFormat.toLowerCase().indexOf("svg")>-1){   // svg + svgz
        
                var exitCode = 0;
                CVJSresponse = "{\"completedAction\":\"svg_creation\",\"errorCode\":\"E"+exitCode+"\",\"converter\":\"AutoXchange AX2020\",\"version\":\"V1.00\",\"userLabel\":\"fromCADViewerJS\",\"contentLocation\":\""+contentLocation+"\",\"contentResponse\":\"stream\",\"contentStreamData\":\""+callbackMethod+"?remainOnServer=0&fileTag="+tempFileName+"&Type="+outputFormat+"\"}";
                ...
                if (outputFormat.toLowerCase().indexOf("pdf")>-1){
                    ...
                    var exitCode = 0;
                    CVJSresponse = "{\"completedAction\":\"pdf_creation\",\"errorCode\":\"E"+exitCode+"\",\"converter\":\"AutoXchange AX2020\",\"version\":\"V1.00\",\"userLabel\":\"fromCADViewerJS\",\"contentLocation\":\""+contentLocation+"\",\"contentResponse\":\"file\",\"contentStreamData\":\""+contentStream+"\"}";
            ...
            if (config.cvjs_debug) console.log("finished - AX2020_callback"+CVJSresponse+" "+outputFormat);	
...
router.post('/callapiconversion', (req, res) => {
	...
	try{
		...
		*/	
		...
		if (cvjs_debug) if (config.cvjs_debug) console.log("CALLAPI contentLocation: "+cvjsRequestJSON.contentLocation+"  compress:"+config.cvjs_svgz_compress);
