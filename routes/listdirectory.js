	...
	try {
		...
		if (directory.indexOf(config.ServerFrontEndUrl) == 0) {
			...
			inputFile =
				...
				directory.substring(config.ServerFrontEndUrl.length);
		} else {
			if (directory.indexOf(config.ServerUrl) == 0) {
				// the ServerUrl is replaced with ServerLocation
				directory =
					config.ServerLocation + directory.substring(config.ServerUrl.length);
			}
		}
		...
		if (directory.indexOf(config.cvjs_Angular_path) == 0) {
			...
			directory =
				config.ServerLocation +
				directory.substring(config.cvjs_Angular_path.length);
		}
		...
		for (var i in files) {
			...
			if (!(name.indexOf(".rw") > 0) && name.indexOf(".") > 0)
				dir_content = dir_content + "<br>" + name;
		}
