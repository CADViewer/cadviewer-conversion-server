...
router.get('/*', function (req, res) {
	...
	try {
		...
		if (cvjs_debug) console.log(config.version+" First in directload2");	
		...
		if (cvjs_debug) console.log("loadfile2 - inputFile = "+ inputFile);	

		var fs = require('fs');

        // 9.20.1
        if(!fs.existsSync(inputFile)) {
            if (cvjs_debug) console.log("error -loadfile2 - file does not exist");
            res.send("Error-directload-01");  // we send empty response 9.20.1
        }
        else{
                // Query the entry
                ...
                if (stats.isDirectory()) {
                    if (cvjs_debug) console.log("error -loadfile2 -folder not file");
                    res.send("Error-directload-02");  // we send empty response 9.10.1
                }
                else{
                    ...
                    fs.readFile( inputFile, null, function(err, data) {
                        if (err){
                            ...
                            if (inputFile.indexOf(".svg")>0 && inputFile.indexOf(".svgz") == -1  ){
            
                                    res.setHeader('Content-Type','image/svg+xml'); // If needed	
                                    res.send(data);	    
                            }
                            else
                            if (inputFile.indexOf(".svgz")>0){   // NOTE: we to this check after .svg         
