...
//    inputFile = config.ServerLocation + "/content/drawings/svg/lightning2a.txt";
    ...
    //res.send(mime.lookup(inputFile));
    //if (true) return;

    var fs = require('fs');
   //fs.readFile( inputFile, 'utf8', function(err, data) {
    fs.readFile( inputFile, function(err, data) {
        //        fs.readFile( inputFile, 'binary', function(err, data) {
        if (err){
            //throw err;
...
//       res.end(data);
       ...
       //res.set({ 'content-type' : 'image/svg+xml' , 'content-encoding' : 'gzip' });
...
//       res.set('Content-Encoding', 'gzip');
//       res.set('Content-Type', 'image/svg+xml');
       // if (config.cvjs_debug) console.log(data);
       //next();
